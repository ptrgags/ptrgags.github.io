...
import {
  MusicAlbum,
  type MusicAlbumDescriptor,
  type TrackDescriptor,
} from '../../src/core/MusicAlbum'
import CardImage from '../../src/components/CardImage.vue'

const { frontmatter } = useData()

const album = new MusicAlbum(frontmatter.value as MusicAlbumDescriptor)

const player = ref<HTMLAudioElement>()

const selected_track = ref(album.first_track)
...
function get_track_classes(track: TrackDescriptor): string[] {
  if (selected_track.value && track.title === selected_track.value.title) {
    ...
    const is_looping = album.play_style === 'loop'
    if (is_looping) {
...
function select_track(track: TrackDescriptor, index: number) {
  ...
  const audio_element = player.value
  if (audio_element) {
    audio_element.src = album.get_track_url(track) ?? ''
    audio_element.load()
...
function next_track() {
  const tracks: TrackDescriptor[] = album.tracks ?? []
