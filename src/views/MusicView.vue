...
import { ALBUMS } from '@/data/music_albums.ts'
import { computed } from 'vue'

const all_albums = computed<Thumbnail[]>(() => {
  const albums = ALBUMS.map((x) => x.thumbnail)

  return albums.filter((x) => !x.hide).sort(sort_reverse_chronological)
})
</script>

<template>
  <div class="one-column plaque">
    <h1 class="center-text">Music</h1>
    <p>
